...
> `console read` 는 페이지가 **보고하는 것**을 그대로 보고합니다 — `console.*` 호출과 실패한 `console.assert` 뿐 아니라 스택 끝까지 올라간 예외와 아무도 처리하지 않은 rejection 까지, 메시지는 모두 브라우저 자신의 문구입니다. 각 항목은 `source`(`console`/`exception`/`rejection`)로 구분됩니다. 기록하지 않는 둘: 페이지가 이벤트를 취소해 브라우저 자신도 출력하지 않은 에러, 그리고 로드에 실패한 서브리소스 — 후자의 페이지 쪽 이벤트는 이유를 담지 않고 네트워크 모니터는 `fetch`/XHR 만 봅니다. 헤드리스에서는 훅이 문서마다 그 문서의 첫 스크립트보다 **먼저** 설치되므로 로드 중 출력도 잡히지만, 브라우저 모드는 사용자의 실제 Chrome 을 몰기 때문에 내비게이션이 정착한 뒤 주입하며 그 페이지 자신의 시작 시점 출력은 잡지 않습니다. 버퍼는 페이지의 전역이라 적대적 페이지는 우회할 수도, 직접 항목을 써넣을 수도 있습니다(던진 적 없는 `exception` 포함). 빈 버퍼를 “아무 일도 없었다”는 증거로도, 채워진 버퍼를 “정말 그랬다”는 증거로도 삼지 마세요.
