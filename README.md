...
> 헤드리스에서는 훅이 문서마다 그 문서의 첫 스크립트보다 **먼저** 설치되므로 페이지가 로드 중 남긴 콘솔·fetch 도 잡힙니다. 브라우저 모드는 사용자의 실제 Chrome 을 몰기 때문에 내비게이션이 정착한 뒤 주입하며, 그 페이지 자신의 시작 시점 출력은 잡지 않습니다. 잡는 대상은 `console` API 와 `fetch`/XHR 입니다 — 잡히지 않은 예외, 처리되지 않은 rejection, 로드에 실패한 서브리소스는 DevTools 에는 보이지만 `console read` 에는 없습니다. 그리고 MAIN 월드 훅이라 적대적 페이지는 우회할 수 있으니, 빈 버퍼를 “아무 일도 없었다”는 증거로 삼지 마세요.
