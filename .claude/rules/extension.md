...
- **Capture** fails loud with typed errors (headless parity); only screenshots degrade explicitly via `screenshot_error`. Full-page is one CDP `captureBeyondViewport` shot — there is no tiling. An action's `--capture` snapshots the tab the agent will act on next (a click-adopted popup included), waiting bounded for a navigated document to parse; its failure never fails the action (a retry would re-run the side effect) and is reported as `capture_error`.
